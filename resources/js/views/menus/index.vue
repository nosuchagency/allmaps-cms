            ...
            <toolbar>
                <template slot="left">
                    <div class="title-icon-wrapper">
                        ...
                        <label>{{$t('menus.plural')}}</label>
                    </div>
                </template>
                <template slot="right">
                    <el-tooltip effect="dark"
                                :content="$t('general.actions.create', {name : $t('menus.singular')})"
                                placement="top-start"
                                v-if="$auth.user().hasPermissionTo('menus.create')">
        ...
        <template slot="content">
            ...
            <div class="content">
                ...
                <el-table :data="tableItems"
                          @row-click="$router.push({name: 'menus-show', params: {id: $event.id}})"
...
<script>
    ...
    export default {
        ...
        computed: {
            text() {
                let name = this.selectedItems.length > 1 ? this.$t('menus.plural') : this.$t('menus.singular');
