            ...
            <toolbar>
                <template slot="left">
                    <div class="title-icon-wrapper">
                        ...
                        <label>{{$t('users.plural')}}</label>
                    </div>
                </template>
                <template slot="right">
                    <el-tooltip effect="dark"
                                :content="$t('general.actions.create', {name : $t('users.singular')})"
                                placement="top-start"
                                v-if="$auth.user().permissions.includes('users.create')">
        ...
        <template slot="content">
            ...
            <div class="content">
                ...
                <el-table :data="tableItems"
                          @row-click="$router.push({name: 'users-show', params: {id: $event.id}})"
...
<script>
    ...
    export default {
        ...
        computed: {
            text() {
                let name = this.selectedItems.length > 1 ? this.$t('users.plural') : this.$t('users.singular');
